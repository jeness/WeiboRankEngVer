...
# -*- coding: utf-8 -*-
"""
train nn and save model, LSTM is built on Keras
"""
import pandas as pd 
import numpy as np 
import jieba
...
def create_dictionaries(model=None,
    ...
    if (combined is not None) and (model is not None):
        ...
        w2vec = {word: model[word] for word in w2indx.keys()}#all index of word vectors with freq>10, (word->model(word))
        ...
        combined= sequence.pad_sequences(combined, maxlen=maxlen)#index to every word in every sentence, when freq < 10, index = 0
...
def get_data(index_dict,word_vectors,combined,y):
    ...
    n_symbols = len(index_dict) + 1  # the number of index of all words, word with fre<10, index=0, so ++1
    embedding_weights = np.zeros((n_symbols, vocab_dim)) # initialize word with index = 0, word vector = 0
